...
class StreampunkPdf(object):
  ...
  def end_namecards(self):
    def namecard_template(canvas, doc):
      canvas.saveState()
      # Fold
      midline = A4L[1] / 2
      canvas.line(0, midline, A4L[0], midline)
      # Logo
      canvas.drawImage(image=doc.streampunk_pdf.logo, x=20, y=20)
      canvas.restoreState()
  ...
  def end_door_listings(self):
    def drinksform_template(canvas, doc):
      canvas.saveState()
      canvas.drawImage(image=doc.streampunk_pdf.logo, x=20, y=20)
      canvas.restoreState()
  ...
  def door_listing(self, day, room, items):
    ...
    self.story.append(Table(data, style=self.tstyle, colWidths=[ inch, None ]))
    self.story.append(PageBreak())
